  ...
  return (
    ...
    >
      <div className="h-full justify-between flex flex-col">
        <div>
          <div onClick={handleToogle} className={`py-5 px-5 cursor-pointer flex gap-3`}>
            <MenuOutlined className="font-semibold" />
            <span className={`${minimize && "hidden"} text-[13px] delay-1000 `}>
              Support Coordinator
            </span>
          </div>
          <SidebarItems minimize={minimize} />
        </div>
        <div className={`py-5 px-5 flex gap-5 hover:text-white hover:bg-primary-danger opacity-75 cursor-pointer`}>
          <LogoutOutlined />
...
const sidebarItems: SidebarItem[] = [
  { link: routes.dashboard, title: "Dashboard", icon: <DashboardOutlined style={{ fontSize: 14 }} /> },
  {
